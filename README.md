...
local BYPASS_SWING_CD = 0.35
local BYPASS_HIT_DIST = 8

local BYPASS_BAT_LIST = {
...
local function tryBypassSwing()
    if bypassHitCD then return end
    bypassHitCD = true
    pcall(function()
        local char = LP.Character
        if char then
            local bat = findBypassBat()
            if bat then
    ...
    task.delay(BYPASS_SWING_CD, function() bypassHitCD = false end)
end
...
local function startBypassAimbot()
    ...
    if hum then
        if bypassPrevAutoRotate == nil then bypassPrevAutoRotate = hum.AutoRotate end
    ...
    bypassAimbotConn = RunService.RenderStepped:Connect(function()
        ...
        for _, plr in ipairs(Players:GetPlayers()) do
            if plr ~= LP and plr.Character then
                ...
                if tRoot and tHum and tHum.Health > 0 then
                    ...
                    if dist < minDist then minDist = dist; target = tRoot end
                end
            end
        end
        ...
        if minDist <= BYPASS_HIT_DIST then tryBypassSwing() end
    end)
end
...
local function stopBypassAimbot()
    ...
    if hum then hum.AutoRotate = (bypassPrevAutoRotate == nil) and true or bypassPrevAutoRotate end
end
...
local function showDiscordInProgressBar()
	...
	progressPct.Text = "discord.gg/ghosthub"
	progressPct.TextColor3 = Color3.fromRGB(255, 0, 0)
...
for _, name in pairs({"GhostHubGUI"}) do
...
local function makeDraggable(frame, isMobileBtn)
local dragging, dragInput, dragStart, startPos = false, nil, nil, nil
frame.InputBegan:Connect(function(inp)
if uiLocked then return end
if isMobileBtn and not State.editMode then return end
...
local _C={
	...
	[7]=Color3.fromRGB(255,0,0),[8]=Color3.fromRGB(160,160,160),
...
verLbl.Text = "discord.gg/ghosthub"
...
portImg.Image = "rbxassetid://122660617006191"
...
byTag.Text = "discord.gg/ghosthub"
...
makeSecHeader("Bypass", "Bypass Features")
rowToggle("Bypass", "Bypass Aimbot", "Enable specialized bypass aimbot", false, function(on)
    bypassAimbotToggled = on
    if on then startBypassAimbot() else stopBypassAimbot() end
end)
rowInput("Bypass", "Bypass Hit Distance", "Distance to trigger swing", BYPASS_HIT_DIST, function(v)
    BYPASS_HIT_DIST = v
end)

makeSecHeader("Settings", "HUD Customization")
rowToggle("Settings", "Edit Mobile Buttons", "Enable moving individual mobile buttons", false, function(on)
    State.editMode = on
    for _, b in pairs(MobilePanel:GetChildren()) do
        if b:IsA("TextButton") then
            ...
            if on then
                ...
                if b:FindFirstChild("EditStroke") then b.EditStroke:Destroy() end
            end
        end
    end
end)
...
rowToggle("Settings", "Show Progress Bar", "Toggle HUD progress bar visibility", true, function(on)
    if pbFrame then pbFrame.Visible = on end
end)

rowToggle("Settings", "Show Mobile Panel", "Toggle the mobile buttons panel", true, function(on)
    if MobilePanel then MobilePanel.Visible = on end
end)

rowActionBtn("Settings", "Change HUD Color", function()
    -- Ciclo de cores para o HUD
    local colors = {Color3.fromRGB(255, 0, 0), Color3.fromRGB(0, 255, 0), Color3.fromRGB(0, 0, 255), Color3.fromRGB(255, 255, 255), Color3.fromRGB(255, 255, 0)}
    local names = {"Red", "Green", "Blue", "White", "Yellow"}
    getgenv().GhostHUDColorIdx = (getgenv().GhostHUDColorIdx or 1) + 1
    if getgenv().GhostHUDColorIdx > #colors then getgenv().GhostHUDColorIdx = 1 end
    
    local newCol = colors[getgenv().GhostHUDColorIdx]
    if progressFill then progressFill.BackgroundColor3 = newCol end
    if progressPct then progressPct.TextColor3 = newCol end
    if progressRadLbl then progressRadLbl.TextColor3 = newCol end
end)
...
do
	...
	local Q_TEXT_OFF = Color3.fromRGB(255, 0, 0)
                ...
                for _, child in ipairs(page:GetChildren()) do
                    if child:IsA("Frame") then
                        for _, subchild in ipairs(child:GetChildren()) do
                            if subchild:IsA("TextBox") and subchild.Parent.Name ~= "GhostHubGUI" then
...
local function setupChar(char)
    ...
    if head then
        ...
        speedLbl.Text="0.0"; speedLbl.TextColor3=Color3.fromRGB(255,0,0)
        ...
        discordLbl.BackgroundTransparency=1; discordLbl.Text="discord.gg/ghosthub"
        discordLbl.TextColor3=Color3.fromRGB(255,0,0); discordLbl.Font=Enum.Font.GothamBold
